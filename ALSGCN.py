...
class DGC(nn.Module):
    def __init__(self):
        ...
        # self.hidden_size = hidden_size  # 节点特征的维度
    ...
    def forward(self, h, adj):
        ...
        """
        # 第1步：使用 softmax 对动态邻接矩阵 adj 进行归一化
        adj_exp = torch.exp(adj)
        lambda_ij_t = adj_exp / adj_exp.sum(dim=1, keepdim=True)  # 公式 (8)

        # 第2步：使用归一化权重聚合节点特征
        h_dt_hat = torch.matmul(lambda_ij_t, h)  # 公式 (9)
